...
function testKeyWords2(keyarr, str, diffsuffixlen) {
  ...
  for (var i = 0; i < keyarr.length; i++) {
    ...
    if (j >= sarr.length && diffsuffixlen) {
      // Если полного совпадения нет - то частичное: indexOf==0 - нач. с keyw->OK
      j = 0;
      // while (j < sarr.length && !weakEq(sarr[j], keyw, diffsuffixlen)) {
      //  j++;
      // }
...
/*
  ...
  var sarr, delim;
    ...
    sarr = scen.split(delim);
    return ( sarr && (sarr.length == 2)) ? delim : '';
  }
  */

/*
  function getActStateIndex(oper) {
    if (oper) {
      if (oper == 'off') return 0;
      if (oper == 'on') return 1;
      if (oper.substr(0,3) == 'set') return 1;
    }	
    return -1;
  }
  */
/*
  function getObjFromAction(str) {
  var sarr;
  
    if ( isDnAction( str ) ) {
      sarr = str.split('.');
      if (sarr && (sarr.length == 2) ) return {dn:sarr[0], act:sarr[1], actStateIndex:getActStateIndex(sarr[1])};
    }
  }
  */
