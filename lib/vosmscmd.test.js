...
function createVo(set) {
  ...
  vo.setLang('ru');
  const dup = vo.addDevices(set, {gendevcmd:true, gengroupcmd: true, typesgroupcmd:["510","520","530"], wordgroupcmd:"свет"});
...
describe('Verbal Command', () => {
  describe('Set1', () => {
    ...
    it('Command exists - включи бра в холле', () => {
      const vo = createVo(set1);

      const result = vo.getActionAndAnswer('включи бра в холле');
      console.log('RESULT='+result)
      assert.equal(typeof result, 'object');
      assert.equal(result.scen, 'LAMP1.on');
    });

    it('Command exists - включи бра в спальне', () => {
      const vo = createVo(set1);

      const result = vo.getActionAndAnswer('включи бра в спальне');
      console.log('RESULT='+result)
      assert.notEqual(typeof result, 'object');
    });
  ...
  describe('Set2', () => {
    ...
    it('Command exists - включи тёплый пол в санузле второго этажа', () => {
      const vo = createVo(set2);
