...
/**
 * На базе списка устройств построить структуры this.locationWords с ключевыми словами уровней и помещений:
 ...
 */
Vosmscmd.prototype.formLocationList = function(devices) {
  ...
  this.markLongnames();
  this.locationWords.places = buildMap(this.places, this.lang);
  this.locationWords.zones = buildMap(this.zones, this.lang);
  this.formTypeWords();
};
 ...
 */
Vosmscmd.prototype.addFirst = function(devices) {
  const res = [];
...
Vosmscmd.prototype.formDnCommands = function(dobj) {
  ...
  const devnameArr = grammar.getKeyWords(dobj.name, this.lang, true); // true - Именительный падеж - беглые гласные не убирать
...
function buildMap(data, lang) {
  ...
  Object.keys(data).forEach(id => {
    ...
    if (item.name) {
      ...
      res.set(item.id, grammar.getKeyWords(str, lang)); // Возвращает массив, так как могут быть варианты!!
    }
  });
  return res;
}
