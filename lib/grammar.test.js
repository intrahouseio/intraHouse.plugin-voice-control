...
describe('Grammar', () => {
  ...
  describe('getKeyWords', () => {
    ...
    it('String with FluentVowel - санузел', () => {
      let name = 'санузел';
      let lang = 'ru';
      const result = grammar.getKeyWords(name, lang, 'F');
      console.log(util.inspect(result));
      assert.equal(typeof result, 'object');
      assert.equal(result.length, 2);
  ...
  describe('cutInfinitiveEnding', () => {
    it('открыть => откр', () => {
      let name = 'открыть';
      let lang = 'ru';
      const result = grammar.cutInfinitiveEnding(name, lang);
      console.log(util.inspect(result));
      assert.equal(result, 'откр');
    });
    it('включить => включ', () => {
      let name = 'включить';
      let lang = 'ru';
