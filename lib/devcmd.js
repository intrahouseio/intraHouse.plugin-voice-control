...
Devcmd.prototype.processDevices = function(devices) {
  this.clearLists();
...
Devcmd.prototype.formDnCommands = function(dobj) {
  ...
  if (multi) {
    ...
    placeArr.forEach(placement => {
      this.wordy.push( this.formDnWordyObj(dobj,  'on', placement));
      this.wordy.push( this.formDnWordyObj(dobj,  'off', placement));
    });
    ...
    this.wordy.push( this.formDnWordyObj(dobj,  'on', ''));
    this.wordy.push( this.formDnWordyObj(dobj,  'off', ''));
  }
...
Devcmd.prototype.formDnWordyObj = function(dobj,  act, placement) {
  let verb = grammar.getActVerb(act, this.lang, dobj.name);
  if (this.lang == 'ru') verb += 'и';

  return {
    id: dobj.dn + '.' + act,
    scene: dobj.dn + '.' + act,
    dn:dobj.dn,
    words: placement + ' ' + dobj.name + ' ' + verb
