...
Vosmscmd.prototype.getChannels = function() {
  return vosmsCommand
    ...
    .map(item => ({ id: item.scen, scene: item.scen, dn: item.dn, words: item.keywords }))
    .sort(hut.byorder('id', 'A'));
};
...
Vosmscmd.prototype.addExt = function(sarr) {
  ...
  sarr.forEach(item => {
    ...
    warr.forEach(keywords => {
      ...
      res.push({
        scen: formExtCommand(item),
        keywords,
        filter,
        value,
        ext: 1,
        reply: grammar.formExtReply(item.name, this.lang)
      });
    });
  });
  this.add(res);
